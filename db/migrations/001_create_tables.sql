...
-- 001_create_tables
--
-- Initial schema for Handoff. This file is the authoritative definition of the
-- database; `db/schema.sql` points here rather than duplicating it.
--
-- Applied by `apps/api/src/db/migrate.ts`, which wraps every migration in a
-- transaction and records it in `migrations_log`. Do not add BEGIN/COMMIT here,
-- and do not edit this file once it has been applied anywhere — the runner
-- checksums applied migrations and will refuse to continue if one changes.
-- =============================================================================
...
-- Deadline queries ("overdue", "due this week") only ever concern open work.
...
COMMENT ON COLUMN tasks.due_date IS 'Optional deadline as an instant; NULL means no deadline.';
